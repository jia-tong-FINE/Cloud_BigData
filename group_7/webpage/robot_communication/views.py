    ...
    if request.method=='GET':
        ...
        if len(number_list)!=2:
            ...
            if userId<1 or userId>610:
                ...
                robot_responses += "为用户"+str(userId)+"推荐以下"+str(movie_num)+"部电影："+"<br>"
                for index in range(len(rate_list)):
                    item=rate_list[index]
                    movie_name_Id=str(item.product)
                    movie_name=movie_dict[movie_name_Id]
                    robot_responses+="第"+str(index+1)+"部：《"+movie_name+"》，评分为"+str(item.rating)[:4]+'\n'
        ...
        if len(number_list)!=2:
            ...
            if str(movieId) not in list(movie_dict.keys()):
                ...
                for index in range(len(rate_list)):
                    ...
                    movie_name_Id=str(item.product)
                    movie_name=movie_dict[movie_name_Id]
                    robot_responses+="为用户"+str(item.user)+"推荐电影：《"+movie_name+"》，评分为"+str(item.rating)[:4]+'\n'
                    if index != len(rate_list):
                        robot_responses+="<br>"
                spark_context.stop()
